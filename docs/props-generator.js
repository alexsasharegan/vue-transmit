const { noop, identity } = require("lodash")
...
const propTable = [`|Property|Type|Default|`]
propTable.push(`|---|---|---|`)
for (const prop in props) {
  const re = /\[object ([a-zA-Z]+)\]/
  let { type, default: propDefault } = props[prop]
  type = Array.isArray(type) ? type : [type]
  type = type.map(t => t.name).join(", ")
  propDefault = typeof propDefault === "function" ? propDefault.name : JSON.stringify(propDefault, null, 2)
  propTable.push(`|${prop}|${type}|${propDefault}|`)
}

console.log(propTable.join("\n"))
